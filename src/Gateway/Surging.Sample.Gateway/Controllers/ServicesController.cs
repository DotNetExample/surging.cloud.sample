        ...
        public async Task<ServiceResult<object>> Path([FromServices]IServicePartProvider servicePartProvider, string path, [FromBody]Dictionary<string, object> model)
        {
            ...
            //{
                if (path == GateWayAppConfig.AuthorizationRoutePath)
                {
                    ...
                    {
                        result = new ServiceResult<object> { IsSucceed = false, StatusCode = MessageStatusCode.RequestError, Message = ex.Message };
                    }
                    catch (Exception ex)
                    {
                        result = new ServiceResult<object> { IsSucceed = false, StatusCode = MessageStatusCode.RequestError, Message = ex.Message };
                    ...
                    if (OnAuthorization(path, rpcParams, ref result))
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (data == null)
                                {
                                    return new ServiceResult<object> { IsSucceed = false, StatusCode = MessageStatusCode.UnAuthentication, Message = "服务异常" };
                                }
                                return CreateServiceResult(data);
                            }
                        }
                        ...
                        {
                            return new ServiceResult<object> { IsSucceed = false, StatusCode = MessageStatusCode.RequestError, Message = ex.Message };
                        }
                        catch (Exception ex)
                        {
                            return new ServiceResult<object> { IsSucceed = false, StatusCode = MessageStatusCode.RequestError, Message = ex.Message };
